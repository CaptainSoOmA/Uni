...
// (c) Christian Maurer   v. 120320 - license see murus.go
...
func pointed (s Size, b byte, L, C uint) bool {
//
//  if L >= charheight (F) { Terminate(); panic ("L zu groß: " + strconv.Uitoa(L)) }
//  if C >= charwidth (F) { Terminate(); panic ("C zu groß: " + strconv.Uitoa(C)) }
  switch s { case Tiny:
    return f07 [b][L][C] != ' '
  case Small:
    return f10 [b][L][C] != ' '
