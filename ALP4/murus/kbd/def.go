...
// (c) Christian Maurer   v. 130403 - license see murus.go
...
type
  ...
  Help; LookFor           // to induce context dependent reactions of the system
  Act; Cfg;               // and for special purposes
  Mark; Demark            // to (de-)select objects
  Deposit; Paste          // "waste paper basket"-operations "copy & paste"
  Black; Red; Green; Blue // to handle colours
  PrintScr; Roll; Pause   // for special purposes
...
//^^^^  ^^^^  ^^^^ TODO names
  Navigate                // to navigate in space with a 3d-mouse
...
/* Commands may be enforced in the "depth" of their "impact":
   ...
   - Help, ..., Paste:        F1-, ..., F8-key
   - Black, ..., Blue:        F9-, ..., F12-key
...
func LastCommand (d *uint) Comm { return lastCommand(d) }
