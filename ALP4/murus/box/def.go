...
// The resulting x has width 0, the colours of the screen and the default editor mode.
// New () *Imp
...
// x has the fore-/backgroundcolour c.
  ColourF (c col.Colour)
  ColourB (c col.Colour)

// x is filled with an empty string.
  Clr (l, c uint)

// Pre: l < NLines, c + width of x <= scr.NXNColumns, c + len(s) <= scr.NColumns.
...
// Pre: y <= scr.NY - scr.NY1.
//      x + scr.NX1 * width of X < scr.NColumns,
//      x + scr.NX1 * Länge of T < scr.NColumns.
...
//      width of x = 0 oder es gilt Länge von T <= width von X.
// If width of x was 0, now width of x == len(s).
...
// Pre: y <= scr.NY - scr.NY1, x + scr.NX1 * width of X < scr.NColumns, x + scr.NX1 * len(s) < scr.NColumns.
// Like Write, starting at pixelpos (column, line) == (x, y).
  EditGr (s *string, x, y uint) // bug
}
